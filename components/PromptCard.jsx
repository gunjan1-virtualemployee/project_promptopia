...
const PromptCard = ({ key, post, handleTagClick, handleEdit, handleDelete }) => {
  ...
  return (
    <div className='prompt_card'>
      <div className='flex justify-between items-start gap-5'>
        ...
        items-center gap-3 cursor-pointer'>
          ...
          <div className='flex flex-col'>
            ...
            font-semibold text-gray-900'>{post?.creator?.username}</h3>
            <p
              className='font-inter text-sm
            text-gray-500'
            >{post?.creator?.email}</p>
          </div>
          <div className='cop_btn'
            onClick={handleCopy}>
      ...
      <p className='font-inter text-sm blue_gradient cursor-pointer' onClick={() => handleClick && handleTagClick}>{post?.tag}</p>
      {
